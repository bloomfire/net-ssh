...
class TestIDRSAPKeys < NetSSHTest
  ...
  def test_in_file_no_password
    tmpdir do |dir|
      ...
      ret = Net::BloomfireSSH.start("localhost", "net_ssh_1", { keys: "#{dir}/id_rsa" }) do |ssh|
  ...
  def test_in_file_with_password
    tmpdir do |dir|
      ...
      ret = Net::BloomfireSSH.start("localhost", "net_ssh_1", { keys: "#{dir}/id_rsa",
  ...
  def test_asks_for_passwords_when_read_from_memory
    tmpdir do |dir|
      ...
      #key_manager = Net::BloomfireSSH::Authentication::KeyManager.new(nil, options)
      ...
      Net::BloomfireSSH.start("localhost", "net_ssh_1", options.merge(password_prompt: prompt)) do |ssh|
        ssh.exec! 'whoami'
      end
    end
  end
