...
class TestPassword < NetSSHTest
  ...
  def test_keyboard_interactive_with_good_password
    ...
    ps.expects(:success)
    ret = Net::BloomfireSSH.start("localhost", "net_ssh_1", auth_methods: ['keyboard-interactive'], password_prompt: pt) do |ssh|
      ssh.exec! 'echo "hello from:$USER"'
    end
    assert_equal ret, "hello from:net_ssh_1\n"
  end
  ...
  def test_password_with_good_password
    ...
    ret = Net::BloomfireSSH.start("localhost", "net_ssh_1", auth_methods: ['password'], password_prompt: pt) do |ssh|
      ssh.exec! 'echo "hello from:$USER"'
    end
    assert_equal ret, "hello from:net_ssh_1\n"
  end
  ...
  def test_bad_password_should_throw_auth_invalid
    assert_raises Net::BloomfireSSH::AuthenticationFailed do
      Net::BloomfireSSH.start("localhost", "net_ssh_1", password: "wrong_password", auth_methods: ['password'], non_interactive: true) do |ssh|
        ssh.exec! 'echo "hello from:$USER"'
      end
    end
  end
