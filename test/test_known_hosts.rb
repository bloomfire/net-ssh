...
class TestKnownHosts < NetSSHTest
  def perform_test(source)
    kh = Net::BloomfireSSH::KnownHosts.new(source)
  ...
  def test_missing_then_add
    Tempfile.open('github') do |f|
      ...
      kh = Net::BloomfireSSH::KnownHosts.new(f.path)
  ...
  def test_search_for
    ...
    keys = Net::BloomfireSSH::KnownHosts.search_for('github.com',options)
  ...
  def test_search_for_with_hostname_and_right_ip_with_check_host_ip
    ...
    keys = Net::BloomfireSSH::KnownHosts.search_for('gitlab.com,35.231.145.151',options)
    assert_equal(1, keys.count)
  end
  ...
  def test_search_for_with_hostname_and_wrong_ip_with_check_host_ip
    options = { user_known_hosts_file: path("known_hosts/gitlab"), check_host_ip: true }
    keys = Net::BloomfireSSH::KnownHosts.search_for('gitlab.com,192.0.2.1',options)
    assert_equal(0, keys.count)
  end

  def test_search_for_with_hostname_and_wrong_ip_without_check_host_ip
    options = { user_known_hosts_file: path("known_hosts/gitlab"), check_host_ip: false }
    keys = Net::BloomfireSSH::KnownHosts.search_for('gitlab.com,192.0.2.2',options)
    assert_equal(1, keys.count)
  end

  def test_search_for_with_hostname_only_with_check_host_ip
    options = { user_known_hosts_file: path("known_hosts/gitlab"), check_host_ip: true }
    keys = Net::BloomfireSSH::KnownHosts.search_for('gitlab.com',options)
    assert_equal(1, keys.count)
  end
  ...
  def test_search_for_with_ip_only_with_check_host_ip
    options = { user_known_hosts_file: path("known_hosts/gitlab"), check_host_ip: true }
    keys = Net::BloomfireSSH::KnownHosts.search_for('35.231.145.151',options)
    assert_equal(1, keys.count)
  end
  ...
  def test_search_for_with_hostname_matching_pattern
    ...
    keys = Net::BloomfireSSH::KnownHosts.search_for('subdomain.gitfoo.com',options)
    assert_equal(1, keys.count)
  end

  def test_search_for_with_hostname_not_matching_pattern_1
    options = { user_known_hosts_file: path("known_hosts/misc") }
    keys = Net::BloomfireSSH::KnownHosts.search_for('gitfoo.com',options)
    assert_equal(0, keys.count)
  end

  def test_search_for_with_hostname_not_matching_pattern_2
    options = { user_known_hosts_file: path("known_hosts/misc") }
    keys = Net::BloomfireSSH::KnownHosts.search_for('subdomain.gitmisc.com',options)
    assert_equal(0, keys.count)
  end

  def test_search_for_with_hostname_not_matching_pattern_3
    options = { user_known_hosts_file: path("known_hosts/misc") }
    keys = Net::BloomfireSSH::KnownHosts.search_for('subsubdomain.subdomain.gitfoo.com',options)
    assert_equal(0, keys.count)
  end
  ...
  def test_search_for_then_add
    Tempfile.open('github') do |f|
      ...
      keys = Net::BloomfireSSH::KnownHosts.search_for('github2.com',options)
      assert_equal(0, keys.count)
