...
module NetSSH
  class TestStartOptions < NetSSHTest
    def setup
      ...
      Net::BloomfireSSH::Transport::Session.stubs(:new).returns(mock('transport_session'))
      Net::BloomfireSSH::Connection::Session.stubs(:new).returns(mock('connection_session'))
    end

    def test_start_should_accept_keepalive_option
      assert_nothing_raised do
        options = { keepalive: true }
        Net::BloomfireSSH.start('localhost', 'testuser', options)
      end
    end
    ...
    def test_constructor_should_reject_options_set_to_array_of_nil
      ...
      Net::BloomfireSSH.start('localhost', 'testuser', keys: [ENV['no-such-env-variable']])
    end
